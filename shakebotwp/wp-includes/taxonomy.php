...
function wp_insert_term( $term, $taxonomy, $args = array() ) {
	...
	//Customized	      
	$category = get_category($args['parent']);
	$arr = explode('-',$args['slug']);
	...
	//if($category->parent == 10 && (!in_array(strtolower($arr[0]),$avaiArr))){
        if((!in_array(strtolower($arr[0]),$avaiArr))){
		return new WP_Error('empty_term_name', __('Category type not in a correct format.'));
	}
	//Customized
		...
		if ( ! $slug_provided || $name_match->slug === $slug || $slug_match ) {
			if ( is_taxonomy_hierarchical( $taxonomy ) ) {
				...
				if ( $existing_term ) {*/
					...
					//return new WP_Error( 'term_exists', __( 'The category name already exists.' ), $existing_term->term_id );
				}
			} else {
	...
	if ( false === $wpdb->insert( $wpdb->terms, compact( 'name', 'slug', 'term_group' ) ) ) {
        ...
        $methigh = $_POST['tag-methigh'];
        $metmed = $_POST['tag-metmed'];
        $metlow = $_POST['tag-metlow'];
        $activityicon = $_POST['taxonomy_image'];        
        
        if(!is_numeric($methigh) ||!is_numeric($metmed) || !is_numeric($metlow) ){
            return new WP_Error( 'db_insert_error', __( 'MET values must be numeric' ), $wpdb->last_error );
        }else{
                $wpdb->query($wpdb->prepare("UPDATE $wpdb->terms SET metmed=$metmed,metlow=$metlow,methigh=$methigh,activityicon='".$activityicon."' WHERE term_id=%d",$term_id));
        }//print_r($args);       
