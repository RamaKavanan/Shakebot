...
function paginate_links( $args = '' ) {
	...
	if ( $args['prev_next'] && $current && 1 < $current ) :
		 ...
		 */
		 if($args['type'] == 'list123'){ //Customized
			$c = $current-1;
			$page_links[] = '<a class="next page-numbers" href="#" onclick=chckjs('.$c.')>' . $args['prev_text'] . '</a>';
		}else{
	...
	for ( $n = 1; $n <= $total; $n++ ) :
		if ( $n == $current ) :
			...
			if ( $args['show_all'] || ( $n <= $end_size || ( $current && $n >= $current - $mid_size && $n <= $current + $mid_size ) || $n > $total - $end_size ) ) :
				...
				/** This filter is documented in wp-includes/general-template.php */
				if($args['type'] == 'list123'){ //Customized
					$page_links[] = "<a class='page-numbers' href='#' onclick=chckjs(".$args['before_page_number'] . number_format_i18n( $n ) . $args['after_page_number'].")>" . $args['before_page_number'] . number_format_i18n( $n ) . $args['after_page_number'] . "</a>";
				}else{
	...
	if ( $args['prev_next'] && $current && ( $current < $total || -1 == $total ) ) :
		...
		if($args['type'] == 'list123'){ //Customized
			$c = $current+1;
			$page_links[] = '<a class="next page-numbers" href="#" onclick=chckjs('.$c.')>' . $args['next_text'] . '</a>';
		}else{
