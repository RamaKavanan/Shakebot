...
//        print_r($_POST['slug']);
	...
	//customized
	$category = get_category($_POST['parent']);	
	$arr = explode('-',$_POST['slug']);
	$avaiArr = array('rhi'=>'rhi','skill'=>'skill','endurance'=>'endurance');
	//echo $category->parent." -- ".(!in_array(strtolower($arr[0]),$avaiArr));
	
	if($category->parent == 10 && (!in_array(strtolower($arr[0]),$avaiArr))){		
		$ret = 'wrongFormat';
	}else{		
	...
	//$ret = wp_update_term( $tag_ID, $taxonomy, $_POST ); 
	/*$tid = $_POST['tag_ID'];	
	$cid = get_cat_ID(trim($_POST['name']));
	...
	if($ret == 'wrongFormat'){ //Customized
		$location = add_query_arg( array( 'error' => true, 'message' => 10 ), $location );
		break;
	}
...
$messages['category'] = array(
	...
	10 => __( 'Category type not in a correct format.' ), //customized
...
<div class="form-field form-required term-name-wrap">
	<label for="tag-metlow"><?php _ex( 'MET (Low Intensity)', 'term name' ); ?></label>
	<input name="tag-metlow" id="tag-metlow" type="text" value="" size="40" aria-required="true" />
	<p><?php _e('The “MET value” is used for the Nutrition Calculation.'); ?></p>
<!--        <p><?php echo"maaaax";echo($post_type); ?><p>-->
</div>

<div class="form-field form-required term-name-wrap">
	<label for="tag-metmed"><?php _ex( 'MET (Medium Intensity)', 'term name' ); ?></label>
	<input name="tag-metmed" id="tag-metmed" type="text" value="" size="40" aria-required="true" />
	<p><?php _e('The “MET value” is used for the Nutrition Calculation.'); ?></p>
</div>

<div class="form-field form-required term-name-wrap">
	<label for="tag-methigh"><?php _ex( 'MET (High Intensity)', 'term name' ); ?></label>
	<input name="tag-methigh" id="tag-met" type="text" value="" size="40" aria-required="true" />
	<p><?php _e('The “MET value” is used for the Nutrition Calculation.'); ?></p>
</div>
<?php if ( ! global_terms_enabled() ) : ?>
<div class="form-field term-slug-wrap">
	<!--<label for="tag-slug"><?php //_e( 'Slug' ); ?></label>--> <!-- Customized -->
	<label for="tag-slug"><?php _e( 'Type' ); ?></label>
	<input name="slug" id="tag-slug" type="text" value="" size="40" aria-required="true" />
	<!--<p><?php //_e('The &#8220;slug&#8221; is the URL-friendly version of the name. It is usually all lowercase and contains only letters, numbers, and hyphens.'); ?></p>-->
	<!-- Customized -->
	<p><?php _e('The &#8220;Type&#8221; is used for the Nutrition Calculation. It must be in the mentioned format for the activities(calculation type - category name). The calculation types are rhi, skill, endurance. (Ex: skill-bowling)'); ?></p>
</div>
...
<div class="form-field term-parent-wrap">
	...
	$dropdown_args['depth'] = 2; //Customized
