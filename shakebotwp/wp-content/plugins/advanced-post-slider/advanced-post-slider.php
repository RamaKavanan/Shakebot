...
#advps_container<?php echo $sldshowID;?> .bx-wrapper .bx-pager.bx-default-pager a {
	...
	background: -webkit-gradient(linear, 0 top, 0 bottom, from(#585858), to(#555555));
	background: -moz-linear-gradient(#585858, #555555);
	filter: progid:DXImageTransform.Microsoft.gradient( startColorstr='#585858', endColorstr='#555555',GradientType=0 );
	background-image: -ms-linear-gradient(top, #585858 0%, #555555 100%);
	background-image: -o-linear-gradient(top, #585858 0%, #555555 100%);
      ...
      <?php 
	  		if(has_post_thumbnail()){
				...
				if($advps_custom_thumb){
					...
					echo '<img src="'.$advps_image.'" width="'.$advps_custom_thumb[0]->width.'" height="'.$advps_custom_thumb[0]->height.'" alt="'.get_the_title().'" class="img-responsive" />';
				}
				else
				{
			...
			{
		?>
        		<img src="<?php echo $container['advps_default_image'];?>" class="img-responsive wp-post-image" alt="<?php the_title();?>" />
         <?php
			}
	  ?>
