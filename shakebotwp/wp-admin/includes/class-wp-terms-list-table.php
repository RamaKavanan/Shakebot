...
class WP_Terms_List_Table extends WP_List_Table {
	...
	public function column_name( $tag ) {
		...
		if(($tag->term_id == 10 || $tag->term_id == 6)){ //customized
			$edit_link = '';
			$out = '<strong><a class="row-title">' . $name . '</a></strong><br />';
		}else{
	...
	protected function handle_row_actions( $tag, $column_name, $primary ) {
		...
		if ( current_user_can( $tax->cap->edit_terms ) && ($tag->term_id != 10 && $tag->term_id != 6) ) { //Customized
		...
		if ( current_user_can( $tax->cap->delete_terms ) && $tag->term_id != $default_term  && ($tag->term_id != 10 && $tag->term_id != 6)) //Customized
			$actions['delete'] = "<a class='delete-tag' href='" . wp_nonce_url( "edit-tags.php?action=delete&amp;taxonomy=$taxonomy&amp;tag_ID=$tag->term_id", 'delete-tag_' . $tag->term_id ) . "'>" . __( 'Delete' ) . "</a>";
		
		//if ( $tax->public ) //Customized
		if ( 0 ) //Customized
...
?>
	...
	<?php if ( !global_terms_enabled() ) { ?>
				<label>
					<span class="title"><?php _e( 'Type' ); ?></span>
