...
function edit_user( $user_id = 0 ) {
    ...
    if($_FILES["avtar"]["name"] != ''){		
		...
		if(in_array($avName,$mime)){			
			...
			$errors->add( 'invalid_avtar', __('<strong>ERROR</strong>: Uploaded avatar Image not in the allowed format. Allowed formats are .gif, .jpeg, .jpg, .png, .bmp, .icon'), array( 'form-field' => 'avtar' ) );	
	...
	if($_FILES["bannerfl"]["name"] != ''){		
		...
		if(in_array($avName,$mime)){			
			...
			$errors->add( 'invalid_bannerfl', __('<strong>ERROR</strong>: Uploaded banner Image not in the allowed format. Allowed formats are .gif, .jpeg, .jpg, .png, .bmp, .icon'), array( 'form-field' => 'bannerfl' ) );	
    ...
    if ( !empty( $_POST['age'] ) && !is_numeric($_POST['age'])){
		$errors->add( 'invalid_age', __('<strong>ERROR</strong>: Age must be a numeric value.'), array( 'form-field' => 'age' ) );
		/*$error[] = __('Age must a numeric value.');*/
	}
	
	 if ( !empty( $_POST['weight'] ) && !is_numeric($_POST['weight'])){
		 $errors->add( 'invalid_weight', __('<strong>ERROR</strong>: Weight must be a numeric value.'), array( 'form-field' => 'weight' ) );
		/*$error[] = __('Weight must a numeric value.');*/
	}
         if ( !empty( $_POST['height'] ) && !is_numeric($_POST['height'])){
		 $errors->add( 'invalid_height', __('<strong>ERROR</strong>: Height must be a numeric value.'), array( 'form-field' => 'height' ) );
		/*$error[] = __('Weight must a numeric value.');*/
	}
